...
let getMessage = ( name : string ) : string => 
{
    return `Hello " ${ name ? name : "None name"}`
}
...
let getSubmitButonEnabled = (s:ItemState) : boolean => {
    switch(s){
...
let enabled = getSubmitButonEnabled(ItemState.Approved);
